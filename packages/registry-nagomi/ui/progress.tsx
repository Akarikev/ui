...
function Progress({
  ...
  const progress = (
    <ProgressPrimitive.Root
      data-slot="progress"
      className={cn(
        "relative w-full overflow-hidden rounded-full border border-border/30 bg-muted/50 shadow-inner shadow-black/[0.03]",
        isCompact ? "h-1" : "h-1.5",
        transitionBase,
        !isLabeled && className
      )}
    ...
    >
      <ProgressPrimitive.Track className="size-full">
        <ProgressPrimitive.Indicator
          ...
          className={cn(
            "h-full rounded-full bg-primary shadow-sm shadow-primary/20 transition-all duration-500 ease-out",
            isAnimated &&
